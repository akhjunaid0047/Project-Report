...
\textbf{Input Corpora}
\begin{enumerate}
    \item \textbf{Alumni Free-Text}: Extracted from 8 NLP target survey fields:
    \begin{itemize}
        \item \texttt{job\_responsibilities}
        \item \texttt{tools\_software}
        \item \texttt{skills\_helped\_land\_job}
        \item \texttt{skills\_missing\_from\_curriculum}
        \item \texttt{skills\_acquired\_after\_graduation}
        \item \texttt{unused\_subjects}
        \item \texttt{certifications\_completed}
        \item \texttt{suggestions\_for\_improvement}
    \end{itemize}
    \item \textbf{Syllabus PDF Text}: Extracted via pdfplumber and structured by \texttt{nlp\_engine/\allowbreak syllabus\_parser.py} (Unit headers, Learning Objectives, Topic breakdowns).
\end{enumerate}
...
\textit{Type: REDUCE --- Topic-Driven, Evidence-Positive} --- A syllabus topic is recommended for reduction only when alumni explicitly report that they never used it in practice. This evidence-positive rule avoids the false positives that arise when foundational topics are simply absent from open-ended survey responses.
\begin{itemize}
...
\textit{Type: OVERHAUL --- Bottom-Percentile Courses} --- OVERHAUL is decided at the analytics-run level using a relative bottom-percentile cut across all courses in the run, rather than an absolute CRS threshold. This avoids both false positives in strong departments and false negatives in weak ones.
\begin{itemize}
    \item Condition: the course's CRS lies in the bottom percentile across the run.
